...
export class PanelBuilder {
    ...
    private scaleBy(
        ...
        let xscaledVertices = yscaledVertices.map(v => {
            ...
            //move the thing up by twice its height to calculate scaling factor
            let factor1 = factor(v[1] + faceHeight / 2);
    ...
    private translateAndRotate(
        ...
        if (faceSlope !== null) {
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                let xe1 = res[i][3];
                let ze1 = res[i][5];
                res[i][0] = xs1 * Math.cos(rads) - zs1 * Math.sin(rads);
                res[i][2] = zs1 * Math.cos(rads) + xs1 * Math.sin(rads);
                res[i][3] = xe1 * Math.cos(rads) - ze1 * Math.sin(rads);
                res[i][5] = ze1 * Math.cos(rads) + xe1 * Math.sin(rads);
            }
        }
