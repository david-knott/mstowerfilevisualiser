/**
 * Projective transform used to manipulate the bracing patterns
 */
export class ProjectiveTransform {
    ...
    getNormalizationCoefficients(srcPts, dstPts, isInverse) {
        ...
        try {
            matC = numeric.inv(
                numeric.dotMMsmall(numeric.transpose(matA), matA)
            );
        } catch (e) {
            console.log(e);
...
class numeric {
    ...
    static _foreach2(x, s, k, f) {
        ...
        var i,
            ...
            ret = Array(n);
        for (i = n - 1; i >= 0; i--) {
            ret[i] = numeric._foreach2(x[i], s, k + 1, f);
        }
        return ret;
    }
